...
public class ControlView extends ViewPart implements IPartListener2, EventHandler{
    ...
    public static final String KEY_FTR = "itraceFTR";
    ...
    private void setupControls(IWorkbenchPart part, Control control) {
        ...
        if (control instanceof StyledText) {
        	...
        	System.out.println("PART"+part.getTitle().toString());
        	String ast = part.getTitle().toString();
        
        		setupStyledText((IEditorPart) part, (StyledText) control, ast);
     ...
     */
    private void setupStyledText(IEditorPart editor, StyledText styledText, String ast) {
    	
    	if(ast.contains(".md")){
    		if (styledText.getData(KEY_QA) == null)
    			styledText.setData(KEY_QA, new QAManager(editor, styledText));

    		System.out.println("MD FILE");
    	}
    	if(!ast.contains(".feature")){
    		if (styledText.getData(KEY_AST) == null)
    			styledText.setData(KEY_AST, new AstManager(editor, styledText));
    	}else{
    		if(styledText.getData(KEY_FTR) == null){
    			styledText.setData(KEY_FTR, new AstFtrManager(editor, styledText));
    		}
    	}
    }
